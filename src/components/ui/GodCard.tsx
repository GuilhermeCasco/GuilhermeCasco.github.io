...
export interface GodProps {
  name: string;
  type: 'Grego' | 'Romano' | 'Monstro';
  element: string;
  description: string;
  abilities: string[];
}

export const GodCard: React.FC<GodProps> = (props) => {
      ...
      >
        ...
        <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-3/4 h-3/4 bg-[var(--color-teal-dark)] mask-image-gradient opacity-80 group-hover:scale-105 transition-transform duration-700 flex items-end justify-center">
           <span className="text-9xl opacity-20 filter blur-sm select-none">
             {props.type === 'Grego' ? '⚡' : props.type === 'Romano' ? '🦅' : '🐍'}
           </span>
        </div>
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 border-t border-[var(--color-gold-dim)] bg-[rgba(8,24,26,0.9)] transform translate-y-2 group-hover:translate-y-0 transition-transform duration-300">
          <div className="text-[var(--color-gold)] text-xs tracking-widest uppercase mb-1">{props.type}</div>
          <h3 className="text-2xl font-title text-[var(--color-sky-pastel)]">{props.name}</h3>
        </div>
      </div>
          ...
          <div className="relative w-full max-w-2xl bg-[var(--color-teal-deep)] border-2 border-[var(--color-gold)] p-8 md:p-12 shadow-[0_0_50px_var(--color-gold-dim)] animate-in fade-in zoom-in duration-300">
            ...
            <div className="flex flex-col md:flex-row gap-8">
              <div className="w-full md:w-1/3 border border-[var(--color-gold-dim)] h-64 flex items-center justify-center bg-[var(--color-teal-dark)]">
                <span className="text-6xl">{props.type === 'Grego' ? '⚡' : props.type === 'Romano' ? '🦅' : '🐍'}</span>
              </div>
              
              <div className="flex-1">
                <div className="text-[var(--color-gold)] tracking-widest uppercase mb-2">{props.type} • {props.element}</div>
                <h2 className="text-4xl font-title text-white mb-6 border-b border-[var(--color-gold-dim)] pb-4">{props.name}</h2>
                
                <p className="text-[var(--color-mist)] font-serif italic mb-6 leading-relaxed">
                  {props.description}
                </p>

                <h4 className="text-[var(--color-gold)] uppercase tracking-widest text-sm mb-3">Habilidades</h4>
                <ul className="grid grid-cols-1 gap-2">
                  {props.abilities.map((ability, idx) => (
                    <li key={idx} className="flex items-center gap-2 text-sm text-[var(--color-sky-pastel)]">
                      <span className="text-[var(--color-gold)]">✦</span> {ability}
                    </li>
                  ))}
                </ul>
              </div>
