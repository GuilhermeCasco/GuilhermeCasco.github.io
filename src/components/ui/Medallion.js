...
export function Medallion(props) {
  ...
  return (
    <div
      className={`relative w-64 h-64 flex items-center justify-center ${className}`}
    >
      {/* Outer Ring */}
      ...
      <svg
        ref={gearRef}
        className="w-48 h-48 text-[var(--color-bronze)] opacity-60"
        viewBox="0 0 100 100"
        fill="currentColor"
      >
      ...
      <div
        ref={lightningRef}
        className="absolute z-10 text-[var(--color-gold)] drop-shadow-[0_0_10px_var(--color-gold)]"
      >
